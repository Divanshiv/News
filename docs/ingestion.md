...
| Dedup | `app/services/ingestion.py` | A story is a duplicate when (a) its canonical URL already exists in `stories`, (b) the same source already has a story with the same normalized title within the last 7 days, or (c) a title/description similarity match against another source finds the same event (link-only, no merge — see `docs/dedup.md`). Exact matches skip the item; fuzzy matches add a `secondary` source link to the existing story. |
